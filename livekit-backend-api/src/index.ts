...
function logEnvironmentVariables() {
  ...
  const envVars = [
    'PORT', 'AWS_REGION', 'REDIS_URL', 'S3_BUCKET',
  ...
  envVars.forEach(varName => {
    ...
    if (value) {
      ...
      const maskedValue = ['LIVEKIT_API_KEY', 'LIVEKIT_API_SECRET'].includes(varName) 
...
async function setupRoutes() {
  ...
  app.get('/connection-details', createConnectionDetailsHandler(livekitService));
  ...
  app.post('/get-token', async (req, res) => {
    ...
    try {
      const token = await livekitService.generateToken(roomName, { identity, name: identity });
  ...
  // Recording routes
  const recordingHandlers = createRecordingHandlers(livekitService, s3Client, redisClient);
